...
export namespace MultiStrokeHint {
	...
	const TAGH = `${PREFIX}::MultiStrokeHintH`;
	...
	export class HlttCompilerH implements IHintCompiler {
		constructor(
			private readonly sink: HlttProgramSink,
			private readonly props: MultiStrokeProps
		) {}
		public doCompile() {
			...
			const sink = this.sink;
			this.sink.addSegment(function* ($) {
				const spurBottom = EmBox.Twilights.SpurLeft(props.emBoxName);
				const spurTop = EmBox.Twilights.SpurRight(props.emBoxName);
